      ...
      case 'briefing':
        return (
          <div className="max-w-3xl mx-auto p-6 rounded-lg" style={{ backgroundColor: 'var(--secondary-background)', border: '1px solid var(--details)' }}>
            <h3 className="text-2xl font-bold mb-4" style={{color: 'var(--secondary-title)'}}>Mensagem do Cliente</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {game.suspects.map((s: ISuspect) => (
              <div key={s.id} className="p-4 rounded-lg" style={{ backgroundColor: 'var(--secondary-background)', border: '1px solid var(--details)' }}>
                <Image
                  src={`/suspeitos/suspeito-${s.id}.png`} // Caminho dinâmico para a imagem
                  alt={`Foto de ${s.name}`}
                  width={120} // Largura da imagem
                  height={120} // Altura da imagem
                  className="rounded-full object-cover mb-4 border-2"
                  style={{ borderColor: 'var(--details)' }}
                />
                <h3 className="text-xl font-bold" style={{color: 'var(--secondary-title)'}}>{s.name}</h3>
                <p className="mt-2"><strong>Perfil:</strong></p>
                <p className="mt-1" style={{ color: 'var(--main-text)' }}>{s.profile}</p>
                <p className="mt-1"><strong>Álibi:</strong></p>
                <p className="mt-1" style={{ color: 'var(--main-text)' }}> {s.alibi}</p>
              </div>
            ))}
          </div>
        );
          ...
          <div className="space-y-4">
            {game.evidence.map((e: IEvidence) => (
              <div key={e.id} className="p-4 rounded-lg" style={{ backgroundColor: 'var(--secondary-background)', border: '1px solid var(--details)' }}>
                <h3 className="font-bold" style={{color: 'var(--secondary-title)'}}>[{e.id}] - {e.title}</h3>
                <p className="mt-2 whitespace-pre-wrap" style={{ color: 'var(--main-text)' }}>{e.content}</p>
              </div>
            ))}
          </div>
        );
          ...
          <div className="space-y-4">
             {game.interrogations.map((i: IInterrogation) => (
              <div key={i.suspect} className="p-4 rounded-lg" style={{ backgroundColor: 'var(--secondary-background)', border: '1px solid var(--details)' }}>
                <h3 className="font-bold" style={{color: 'var(--secondary-title)'}}>{i.suspect}</h3>
                <p className="mt-2 italic whitespace-pre-wrap" style={{ color: 'var(--main-text)' }}>"{i.transcript}"</p>
              </div>
            ))}
          </div>
        );
